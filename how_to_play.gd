...
func _on_pressed():
	# Get the CanvasLayer node again, just to be safe (though it could be stored in _ready).
	var instructions_layer: CanvasLayer = get_node(instructions_canvas_layer_path)
	...
	if instructions_layer != null:
		...
		print("ERROR: Could not make Instructions CanvasLayer visible. Node not found.")
